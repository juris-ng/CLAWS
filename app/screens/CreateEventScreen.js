// screens/body/CreateEventScreen.js
...
const CreateEventScreen = ({ navigation }) => {
  ...
  const [showDatePicker, setShowDatePicker] = useState({ visible: false, field: null, mode: 'date' });
  ...
  const categoryOptions = [
    { id: 'meeting', label: 'Meeting', icon: 'people-outline', color: '#2196F3' },
    { id: 'workshop', label: 'Workshop', icon: 'bulb-outline', color: '#FF9800' },
    { id: 'townhall', label: 'Town Hall', icon: 'megaphone-outline', color: '#F44336' },
    { id: 'fundraiser', label: 'Fundraiser', icon: 'cash-outline', color: '#4CAF50' },
    { id: 'social', label: 'Social', icon: 'happy-outline', color: '#9C27B0' },
  ...
  const handleDateTimeChange = (event, selectedValue) => {
    if (event.type === 'set' && selectedValue) {
      updateFormData(showDatePicker.field, selectedValue);
    }
    setShowDatePicker({ visible: false, field: null, mode: 'date' });
  };
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <TextInput
              ...
              placeholder="Describe what the event is about, agenda, and what participants can expect..."
              placeholderTextColor={COLORS.mediumGray}
            ...
            <View style={styles.row}>
              <View style={styles.halfWidth}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setShowDatePicker({ visible: true, field: 'eventDate', mode: 'date' })}
                >
              ...
              <View style={styles.halfWidth}>
                ...
                <TouchableOpacity
                  style={styles.dateButton}
                  onPress={() => setShowDatePicker({ visible: true, field: 'eventTime', mode: 'time' })}
                >
                  <Ionicons name="time-outline" size={20} color={COLORS.purple} />
                  <Text style={styles.dateText}>{formatTime(formData.eventTime)}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
            ...
            <View style={styles.switchRow}>
              <View style={styles.switchLeft}>
                ...
                <View style={styles.switchText}>
                  ...
                  <Text style={styles.switchDescription}>Track attendees and send reminders</Text>
                </View>
              </View>
              <Switch
                value={formData.requiresRSVP}
                onValueChange={(value) => updateFormData('requiresRSVP', value)}
                trackColor={{ false: COLORS.lightGray, true: COLORS.purple + '40' }}
                thumbColor={formData.requiresRSVP ? COLORS.purple : COLORS.white}
              />
            </View>

            {formData.requiresRSVP && (
              <View style={styles.conditionalInput}>
                <Text style={styles.subLabel}>Maximum Attendees (Optional)</Text>
                <TextInput
                  style={styles.input}
              ...
              {visibilityOptions.map((option) => (
                ...
                >
                  <Ionicons
                    name={option.icon}
                    size={18}
                    color={formData.visibility === option.id ? COLORS.purple : COLORS.mediumGray}
                  />
                  <Text
                    style={[
        ...
        {showDatePicker.visible && (
          <DateTimePicker
            ...
            minimumDate={showDatePicker.field === 'eventDate' ? new Date() : undefined}
          />
        )}
      </KeyboardAvoidingView>
    </SafeAreaView>
